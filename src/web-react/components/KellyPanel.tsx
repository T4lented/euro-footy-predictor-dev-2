import { Download, FileUp, Plus, ShieldCheck, Wallet, X } from 'lucide-react';
  ...
  return (
    <section className="border-t pt-5" style={{ borderColor: 'var(--border-glass)' }}>
      ...
      <div className="glass mt-4 p-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><p className="font-display text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>Manual decimal odds</p><p className="mt-1 text-xs" style={{ color: 'var(--text-secondary)' }}>Live provider access is disabled until a server-only key is configured. These inputs work without an account.</p></div>
          <span className="font-mono text-[10px] uppercase tracking-wide" style={{ color: 'var(--text-secondary)' }}>{market ? `Market overround ${market.overroundPercent.toFixed(1)}%` : 'Enter all three prices'}</span>
        </div>
